---
title: "OpenKnowledge 把 Markdown 编辑器重做成「AI 一等公民」"
date: 2026-06-26
description: "OpenKnowledge 把 Claude/Codex/Cursor 当作 Markdown 一等写入者，本地仓库变 LLM Wiki。"
tags: ["AI", "知识管理", "Claude Code", "开源", "Obsidian"]
categories: ["AI"]
keywords: ["OpenKnowledge", "inkeep", "AI-native 编辑器", "LLM Wiki", "Claude Code MCP", "Obsidian 替代"]
cover: /images/covers/2026-06-26-AI-openknowledge-ai-native-markdown-editor.svg
draft: false
toc: true
---

# OpenKnowledge 把 Markdown 编辑器重做成「AI 一等公民」

上周 HN 上一条 Show HN 静悄悄冲到 195 分——`inkeep/open-knowledge`，一个把自己定位为「AI-native markdown editor and LLM Wiki」的项目。它没有去和 Obsidian 比插件数量，也没有在 Notion 的协作赛道上抢位置。它做的事情更激进：**把 Claude、Codex、Cursor 这些 Agent 当作 Markdown 文件的一等写入者，让本地仓库变成多个 LLM 都能挂载的 Wiki**。

23 天，329 stars，195 HN 分。这种「不靠功能堆叠、靠范式迁移」的开源开局，在 2026 年的 AI 工具赛道已经非常罕见了。
...
### 2.1 WYSIWYG Markdown + 协作式 AI 改写
...
技术上的实现路径是：本地启动一个 Web 编辑器，**在同一个文件上同时跑人类的光标和 Agent 的光标**。Agent 不再是「吐一整段 diff 让你手动合并」的旁观者，而是一个可以**实时改写同一个段落、然后人类接着改下一句**的协作写作者。

这套机制的本质，是把「提示词 → 输出」的串行流程，重构成了「共享编辑会话 → 并行修改 → diff 收敛」的并行流程。

### 2.2 MCP + Skills + Agentic Search 三件套

第二个亮点是 out-of-the-box 的 **MCP、skills、agentic search**。具体而言：

- **MCP server**：本地起一个 MCP server，让任何支持 MCP 的 Agent（Claude Desktop、Codex CLI、Cursor 等）都能把当前 Wiki 当作挂载点；
...
这一套组合拳的工程意义在于：**它把「让 Agent 理解你的知识库」从「自己写 RAG、自己写检索 prompt」降级成了 `ok init` 一条命令**。
...
`ok init` 做的事情相当于：扫描当前目录 → 生成 `.openknowledge/` 配置 → 自动在 Claude Code / Cursor / Codex 里挂载对应的 MCP server 和 skill。整个过程不需要手动改 JSON。
...
|------|----------|--------|---------------|
| 主要使用者 | 人类 | 人类 | **人类 + Agent 并列** |
| 存储单元 | 本地 Markdown | 云端 block | 本地 Markdown + 元信息 |
| 协作单位 | 文件 | block / 页面 | commit |
| 历史追溯 | git 插件 | 版本历史 | **原生 git** |
| Agent 接入 | 第三方插件 | API | **一等公民 MCP/skills** |
| 网络依赖 | 离线优先 | 必须联网 | **离线优先** |

**Obsidian 的核心抽象是「文件 + 链接」，Notion 的核心抽象是「block + 数据库」，OpenKnowledge 的核心抽象是「Wiki 节点 + Agent session」**。
...
2. 触发 Claude Code 改写某一节（直接调 `ok` CLI 或者走 MCP）；
...
当然，23 天 329 stars 并不意味着产品形态已经稳定。几个值得观察的点：
...
- **WYSIWYG + Agent 实时协作的并发冲突**：两个人类 + 多个 Agent 同时改同一段 Markdown 时，diff 合并的语义仍是模糊地带；
...
- 项目主页：<https://github.com/inkeep/open-knowledge>
- HN Show HN：<https://news.ycombinator.com/item?id=48675435>
- 官方文档：<https://openknowledge.ai/docs>
- Claude Code 工具集实战笔记（内链）：<https://www.onlythinking.com/post/2026-04-11-tools-serena%E5%BC%80%E6%BA%90ai%E7%BC%96%E7%A8%8Bagent-ide/>
- 小米 MiMo Code 200 步 Coding Agent（内链）：<https://www.onlythinking.com/post/2026-06-17-ai-xiaomi-mimo-code-200-step-coding-agent/>

## 关于作者

编程码农，关注 AI Agent、开发者工具与开源生态。公众号「**编程码农**」同步更新，欢迎交流。
