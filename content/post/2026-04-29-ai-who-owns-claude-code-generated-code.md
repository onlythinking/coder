...
**GPL 污染是怎么发生的？** 大模型的代码生成并非真正的"创作"，而是在训练数据的概率空间中进行采样。当训练数据中包含 GPL 代码时，模型会学习这些代码的特定模式：变量命名习惯、特定算法的高频实现、常见的数据结构组合。当用户请求类似功能的代码时，模型可能在无意中复现这些受版权保护的代码片段——即使没有直接复制粘贴，代码的 AST（抽象语法树）结构和逻辑流程也可能与原始代码构成"实质相似"。

这也是为什么GPL诉讼中原告通常会提交代码结构对比图，而非简单的字符查重。
...
> "You retain ownership of your Input and Output. You are responsible for ensuring that your inputs and use of Claude Code do not infringe any third party's rights."
> 要点：Anthropic 明确将侵权责任转移给用户，且声明训练数据来源为公开数据和用户输入，不对输出代码的版权清洁性提供背书。

**GitHub Copilot（Microsoft）**
> "You are responsible for ensuring your use of Copilot complies with applicable laws and terms, and for any content generated by Copilot."
> 要点：微软同样将合规责任归于用户。但企业版提供"IP 补偿计划"——若企业因 Copilot 生成的代码遭遇第三方版权索赔，微软提供法律费用支持。

**Cursor**
> "You own all content generated by Cursor. You warrant that you have the necessary rights to provide inputs to Cursor."
> 要点：Cursor 条款最简洁，同样将责任归于用户侧。
